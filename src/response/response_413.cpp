...
namespace KalaKit::ResponseSystem
{
		...
		const string& targetContentType)
	{
		...
		string statusLine = "HTTP/1.1 413 Payload Too Large";

		size_t totalSize = 2;
		bool sliced = false;
		vector<char> body = Server::server->ServeFile(
			Server::server->errorMessage.error405,
			0,
			0,
			totalSize,
			sliced);
		...
		if (body.empty())
		{
			PrintData rData =
			{
				...
				.customTag = "RESPONSE",
				.message = "413 response file body was empty!"
			};
			...
			string newBody =
				...
				"		<p>The payload you tried to send exceeded allowed server limits!</p>"
				"	</body>"
				"</html>";
